 ...
 * 3.4 村长立绘接线验证探针
 *
 * 验证：PORTRAIT_MAP 含「村长 → elder.png」映射 + elder.png 资源可加载（无 404）。
...
async function run() {
  ...
  try {
    ...
    // elder.png 资源 HTTP 可访问（无 404）
    const resp = await page.goto(GAME_URL + 'assets/portraits/elder.png', { waitUntil: 'networkidle2' }).catch(() => null);
    check('elder.png HTTP 可访问', !!resp && resp.ok(), resp ? `${resp.status()}` : '<请求失败>');
    ...
    check('运行时村长立绘 img 显示', runtime.played && runtime.src.includes('elder.png'), runtime.src || (runtime.reason || '<无立绘>'));

    // elder.png 尺寸验证（512×768）
    const imgInfo = await page.evaluate(async () => {
      const im = new Image();
      await new Promise(r => { im.onload = r; im.src = '/assets/portraits/elder.png'; });
      return { w: im.naturalWidth, h: im.naturalHeight };
    }).catch(() => null);
    check('elder.png 512×768', imgInfo?.w === 512 && imgInfo?.h === 768, imgInfo ? `${imgInfo.w}x${imgInfo.h}` : '<加载失败>');

 ...
 *   4. 木屋墙完整性：花园恢复不得覆盖 col 2 石墙（gid 3 碰撞），防止玩家穿墙
...
const SNAP = `(() => {
  ...
  return {
    ...
    groundAt: { soil: t(gLD, 1, 19), path: t(gLD, 1, 20), grass: t(gLD, 1, 18) },
    wallAt: { flower1: t(wLD, 1, 18), flower2: t(wLD, 1, 20), flower3: t(wLD, 1, 22) },
    collides: { flowerCollides: col(1, 20), pathCollides: col(1, 19) },
    // 木屋石墙完整性（花园恢复不得破坏 col 2 墙 gid 3 / 碰撞）
    wallIntact: { gid: t(wLD, 2, 20), collides: col(2, 20) },
...
async function run() {
  ...
  try {
    ...
    check('初始 荒土瓦片 (1,19)=gid 2', d.groundAt.soil === 2, `实际=${d.groundAt.soil}`);
    ...
    check('初始 木屋墙 (2,20)=gid 3 且碰撞', d.wallIntact.gid === 3 && d.wallIntact.collides === true,
      `实际=${JSON.stringify(d.wallIntact)}`);
    ...
    check('交互后 荒土清除 (1,18)=gid 1 草地', d.groundAt.grass === 1, `实际=${d.groundAt.grass}`);
    check('交互后 小路 (1,20)=gid 7', d.groundAt.path === 7, `实际=${d.groundAt.path}`);
    check('交互后 花丛 (1,18)/(1,20)/(1,22)=gid 8',
    ...
    check('交互后 木屋墙仍完整 (2,20)=gid 3 且碰撞', d.wallIntact.gid === 3 && d.wallIntact.collides === true,
      `实际=${JSON.stringify(d.wallIntact)}`);
    ...
    check('重进后 花丛仍在 (1,18)=gid 8', d.wallAt.flower1 === 8, `实际=${d.wallAt.flower1}`);
    check('重进后 小路仍在 (1,20)=gid 7', d.groundAt.path === 7, `实际=${d.groundAt.path}`);
    check('重进后 木屋墙仍完整 (2,20)=gid 3 且碰撞', d.wallIntact.gid === 3 && d.wallIntact.collides === true,
      `实际=${JSON.stringify(d.wallIntact)}`);

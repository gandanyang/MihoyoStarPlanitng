 ...
 *
 * 验证（任务卡验收项）：
 *  1. 背包一键出售：二次确认后执行；金币正确累加；可售物品清空
 *  2. 商店一键出售：同上
 *  3. 不可售物品（工具/钥匙/机器人/钻石/种子）不受影响
 *  4. 无可售物品时：按钮置灰 + 点击提示
 *  5. 二次确认：点「全部出售」先弹确认框，确认后才会真正卖出
 *
...
// 期望金币：100 + 萝卜3×15 + 番茄2×35 + 石头1×5 + 木材1×8 = 100 + 45 + 70 + 5 + 8 = 228
const EXPECTED_COINS_AFTER = 228;
...
async function run() {
  ...
  try {
    ...
    // 点「全部出售」→ 应先弹确认框，金币/物品不变
    await page.evaluate(() => {
      const btn = document.querySelector('#backpack-panel [data-action="sell-all"]');
      if (btn) btn.click();
    });
    await sleep(400);
    const confirmOpen = await page.evaluate(() => !!document.querySelector('div[style*="z-index: 980"]'));
    check('点击后弹出确认框（未直接执行）', confirmOpen === true);
    bstate = await backpackState(page);
    check('确认前金币仍是 100G', bstate.coins.includes('100G'), bstate.coins);

    // 点击确认 → 真正卖出
    await page.evaluate(() => {
      const okBtn = document.querySelector('button[data-act="ok"]');
      if (okBtn) okBtn.click();
    });
    await sleep(800);
    bstate = await backpackState(page);
    console.log(`  卖出后: coins="${bstate.coins}" cellNames=[${bstate.cellNames.join(',')}]`);
    check('金币累加至 228G', bstate.coins.includes('228G'), bstate.coins);
    check('可售物品已清空', !bstate.cellNames.includes('萝卜') && !bstate.cellNames.includes('番茄') && !bstate.cellNames.includes('石头') && !bstate.cellNames.includes('木材'));
    ...
    const toastShown = await page.evaluate(() => {
      ...
      return t?.style?.display === 'block' && (t.textContent ?? '').includes('没有可出售的物品');
    });
    check('无可售物品时点击有提示', toastShown === true);
    const coinAfterEmpty = await backpackState(page);
    check('无可售时金币不变（仍 228G）', coinAfterEmpty.coins.includes('228G'), coinAfterEmpty.coins);
    ...
    await sleep(400);
    const shopConfirmOpen = await page.evaluate(() => !!document.querySelector('div[style*="z-index: 980"]'));
    check('商店点击后弹出确认框', shopConfirmOpen === true);
    ...
    check('商店卖出后金币 228G', sstate.coins.replace(/\s/g, '').includes('228G'), sstate.coins);
    check('商店出售栏无可卖按钮（全部置灰）', sstate.sellableButtons === 0, `可卖按钮=${sstate.sellableButtons}`);
    ...
    check('存档无新增字段（player 仅位置/背包）', Array.isArray(saveKeys?.playerKeys) && !saveKeys.playerKeys.some(k => !['x', 'y', 'scene', 'facing', 'inventory'].includes(k)), saveKeys?.playerKeys?.join(','));

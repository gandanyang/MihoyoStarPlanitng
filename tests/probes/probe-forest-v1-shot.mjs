/**
 * forest_tileset_v1 实机效果截图探针
 *
 * 种子存档直达 forest 场景，等待渲染稳定后：
 *   1. 全屏截图（1024×768 视口，横屏）
 *   2. 验证 tiles 纹理已加载 forest_v1（256px = 16 tile）
 *   3. 多视角截图（老树/碎片/小屋区域）
 *
 * 用法：先起 dev server（npm run dev），再 node tests/probes/probe-forest-v1-shot.mjs
 */
...
async function run() {
  console.log('=== forest_tileset_v1 实机截图 ===\n');
  ...
  try {
    ...
    await page.evaluate(() => {
      localStorage.setItem('return_star_save', JSON.stringify({
        version: '0.5', savedAt: 'forest_v1shot', timestamp: Date.now(),
        player: { x: 240, y: 160, scene: 'forest', facing: 'down', inventory: {} },
    ...
    console.log(`tiles 纹理宽 = ${info.tilesW}（期望 256 = forest v1）`);
    console.log(`画面中央采样 = ${info.sample}`);
    if (info.tilesW === 256) console.log('✅ forest v1 tileset 已加载');
    else console.log('❌ tiles 纹理异常');

    const shot = join(SHOT_DIR, 'forest-v1-full.png');
    await page.screenshot({ path: shot });
    console.log(`📸 ${shot}`);

    // 多视角截图（老树区域/碎片位置/入口/小屋区域）
    const zones = [
      ['老树', 128, 128],
      ['碎片', 320, 160],
      ['入口', 240, 280],
    ...
    for (const [name, x, y] of zones) {
      try {
        ...
        await sleep(600);
        const zshot = join(SHOT_DIR, `forest-v1-${name}.png`);

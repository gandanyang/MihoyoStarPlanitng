...
流程（默认 variant=release，可 --variant debug/release/both）：
  ...
  6. 复制 APK 到 dist_apk/（latest-<variant>.apk，每次覆盖）
  7. （可选，--archive）额外复制带时间戳的归档备份到 dist_apk/
...
APK_DST_DIR = ROOT / "dist_apk"  # 默认复制 latest-<variant>.apk 到这里；--archive 额外做时间戳备份
...
def probe_environment() -> dict[str, str]:
    ...
    print(f"  APK 最终输出：{APK_DST_DIR}/latest-<variant>.apk（Gradle 原产：{APK_OUT_ROOT}/<variant>/）")
...
def validate_and_report(variants: list[str], do_archive: bool) -> None:
    ...
    for v in variants:
        ...
        size_mb = p.stat().st_size / (1024 * 1024)
        latest = APK_DST_DIR / f"latest-{v}.apk"
        print(f"    {v:<7} → {latest}   ({size_mb:.1f} MB)")
    print(f"\n  Gradle 原生产物：{APK_OUT_ROOT}/<variant>/")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help="额外复制带时间戳的归档备份到 dist_apk/（默认只复制 latest-<variant>.apk）",
    )
    ap.add_argument(

...
| 建设点 | NPC | 定稿文案 |
|---|---|---|
| 老屋 | 村长 | "你爷爷以前每天都会擦这里。"（→ 完整批次 `OLD_HOUSE_RESTORED_DIALOGUE`） |
...
- [ ] **物品 food**：4 作物 category=food，需求"食物"可用任意作物交付（§1.3 独立项，本轮收尾未实施）
- [x] **对白批次**：老屋→村长、道路→老张各触发一次；花园沿用既有夏雅见证对白（2026-08-06 制作人拍板：不新增小梅，避免语义重复 + 叠加触发）
